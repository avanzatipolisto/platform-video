...
class SqliteUsers:
    ...
    def add_user(self, name, password, email, birddate):
        self.sqliteClient.cursor.execute("insert into users (name, password, email, birddate, rol) values (?, ?, ?, ?,?)", (name, password, email, birddate, "normal"))
        self.sqliteClient.connection.commit()
    def update_user(self, id, name, password, email, birdate, rol):
        self.sqliteClient.cursor.execute(f"update users set name='{name}',password ='{password}', email='{email}', birddate='{birdate}', rol='{rol}'  where id_usuario = '{id}'")
        self.sqliteClient.connection.commit()
